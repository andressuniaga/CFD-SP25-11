...
This report entails simulations of Mach 3 supersonic flow over a step as shown in Figure \ref{details}. We first take a walk through the equations that govern this flow such as a simplified \textit{Navier-Stokes} equations which make up Euler's equations and Normal Shock and Isentropic Flow theory. 
...
\subsection{Mesh hierarchy and time step} %Fix table vaues
\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{MeshID} & \textbf{Cells} & $\Delta t$ [s]\\\hline
Nx1  & 4,032 & 0.006\\  %add in real values
Nx2  & 16,128 & 0.019\\
Nx4  & 64,512 & 0.076\\
Nx8  & 258048 & 0.009\\\hline
\end{tabular}
\caption{\centering Mesh and solver parameters used in the convergence study.\\[2mm] \textbf{NOTE:} Decrease in delta T for Nx8 mesh since that mesh was ran in parallel on 56 processors.}
\label{tab:meshes}
\end{table}

\subsection{Grid‑convergence of key variables} %Generate Grid-convergence of key variables
\begin{figure}[H]
\centering
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\linewidth]{images/p_Nx1.png}
    \caption{meshID: Nx1}
\end{subfigure} 
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\linewidth]{images/p_Nx2.png}
    \caption{meshID: Nx2}
\end{subfigure} 
\vspace{2mm}
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\linewidth]{images/p_Nx4.png}
    \caption{meshID: Nx4}
    \end{subfigure} 
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\linewidth]{images/p_Nx8.png}
    \caption{meshID: Nx8}
    \end{subfigure} 
\caption{Static pressure field in [$\mathrm{Pa}$] at $t=4\; \text{s}$ for meshes Nx1, Nx2, Nx4, Nx8. All panels share identical contour limits (0.1 Pa - 17 Pa) to highlight sharpening of the shock with refinement.}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/rho_Nx1.png}
        \caption{meshID: Nx1}
    \end{subfigure}
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/rho_Nx2.png}
        \caption{meshID: Nx2}
    \end{subfigure}
    ...
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/rho_Nx4.png}
        \caption{meshID: Nx4}
    \end{subfigure}
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/rho_Nx8.png}
        \caption{meshID: Nx8}
    \end{subfigure}
    
    \caption{Static density field in [$\mathrm{kg\,m^{-3}}$] at $t = 4\;\text{s}$ for meshes Nx1, Nx2, Nx4, Nx8.  All panels use identical contour limits to emphasise convergence of density gradients across the normal shock and expansion fan.}
    \label{fig:rho_convergence}
    \end{figure}

In the following sections the pressure is recorded along a line very close to the surfaces (offset 1e-5 meters) across the span of every surface. 
\subsection{Pressure distribution along surfaces}
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/entryWallBottom x-plot.png}
        \caption{Bottom wall (inlet floor)}
    \end{subfigure}
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/entryWallTop x-plot.png}
        \caption{Top wall (tunnel ceiling)}
    \end{subfigure}
    
    \vspace{2mm}
    
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/stepFaceHoriz x-plot.png}
        \caption{Step face — horizontal}
    \end{subfigure}
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/stepFaceVert Pressure y-plot.png}
        \caption{Step face — vertical}
    \end{subfigure}
    
    \caption{Wall-pressure profiles at $t = 4\;\text{s}$ for meshes Nx1 (blue), Nx2 (orange), Nx4 (green), Nx8 (red).  
    The red “$\times$” denotes the theoretical stagnation pressure $p_{0,2}= 12.06\; \text{Pa}$.}
    \label{fig:wallPressureMulti}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=1.095\linewidth]{images/horizSurfaces.png}
        \caption{Horizontal walls}
    \end{subfigure}
    \begin{subfigure}{0.495\linewidth}
        \includegraphics[width=\linewidth]{images/vertSurface.png}
        \caption{Vertical walls}
    \end{subfigure}
    \caption{Converged (Nx8) pressure distribution on horizontal (left) and vertical (right) walls at $t = 4\,\text{s}$.}
    \label{fig:wallPressureSingle}
\end{figure}
        

\subsection{Discussion of convergence}
The leading-edge stagnation pressure on the bottom wall (Fig.~\ref{fig:wallPressureMulti}, upper-left) climbs from $11.3 \; \text{Pa}$ on the coarsest grid to $11.96 \; \text{Pa}$ on Nx8, only $\displaystyle\left|\frac{11.96-12.06}{12.06}\right|\approx 0.8\%$ below the analytical 
value.
\vspace{3mm}

Away from the step, the pressure and density fields exhibit convergence:
peak-to-peak differences becoming smaller and smaller (compare Figs. \ref{fig:p_convergence} and \ref{fig:rho_convergence}).
\vspace{3mm}

Overall, the Nx4 mesh provides very high accuracy, while Nx8 serves as a reference solution for quantitative validation. All subsequent comparisons in this report therefore use the Nx8 data set.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Parallel Performance Metrics Using Different \# of Processors}
...
The collected data and graph clearly demonstrate that initially, as the number of processors increases, the speedup closely follows the ideal linear scaling, indicating efficient parallelization. This nearly linear behavior occurs because the computational workload is sufficiently large to ensure that each processor remains fully utilized, with minimal communication overhead and synchronization requirements. 
However, as the number of processors continues to increase beyond a certain point (around 16-32 processors in our tests), we observe diminishing returns in the speedup curve. This slowdown in parallel performance may occur due to several factors including: communication overhead, more processors need an increase in communication between them resulting in additional computational overhead, and synchronization delays, as processors counts rise, synchronizing the solver across multiple process introduces small yet accumulating errors.
\vspace{3mm}

This analysis highlights the importance of identifying an optimal number of processors to maximize computational efficiency. While adding processors initially leads to significant speed improvements, the overhead and inherent limitations eventually offset these gains. Thus, performance characterization, as demonstrated by our data, plays a crucial role in practical decision-making for deploying parallel computations effectively.
